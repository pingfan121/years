...
namespace Easy4net.Common
{
    ...
    {
        public static Dictionary<Type, PropertyInfo[]> cache=new Dictionary<Type,PropertyInfo[]>();
        public static PropertyInfo[] GetProperties(Type type)
        {
            if (!cache.ContainsKey(type)) {
                cache[type]=type.GetProperties(BindingFlags.Public | BindingFlags.Instance);
            }
            return cache[type];
        }
